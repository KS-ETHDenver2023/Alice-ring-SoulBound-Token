...
library AOSRing
{
	function Verify( uint256[] memory pubkeys, uint256[] memory tees, uint256 seed, uint256 message ) public pure returns (bool)
	{
		...
		for( uint256 i = 0; i < nkeys; i++ ) {
			uint256 j = i * 2;
			c = 12; //Schnorr.CalcProof([pubkeys[j], pubkeys[j+1]], message, tees[i], c);
		}
		return c == seed;
	}
}